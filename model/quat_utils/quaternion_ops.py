...
class QuaternionDeconv2D(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, output_padding):
        super(QuaternionDeconv2D, self).__init__()
        
        # Apply a separate deconvolution for each quaternion component
        self.deconv_real = nn.ConvTranspose2d(in_channels, out_channels, kernel_size, stride, padding, output_padding)
        self.deconv_i = nn.ConvTranspose2d(in_channels, out_channels, kernel_size, stride, padding, output_padding)
        self.deconv_j = nn.ConvTranspose2d(in_channels, out_channels, kernel_size, stride, padding, output_padding)
        self.deconv_k = nn.ConvTranspose2d(in_channels, out_channels, kernel_size, stride, padding, output_padding)

    def forward(self, x):
        # Apply deconvolution separately for each component
        real = self.deconv_real(x)
        i = self.deconv_i(x)
        j = self.deconv_j(x)
        k = self.deconv_k(x)

        # Stack the results back into a quaternion
        return torch.cat([real, i, j, k], dim=1)  # Shape: [batch, channels, height, width, 4]

class QuaternionUpsampler1D(nn.Module):
    def __init__(self, kernel_size, scale, n_feat, bn=False, act=False, bias=True, dropout_prob=0.2):
        super(QuaternionUpsampler1D, self).__init__()

        # Use quaternion-aware deconvolution layer
        self.deconv_layer = QuaternionDeconv2D(
            in_channels=n_feat,
            out_channels=2 * n_feat,  # Number of output channels is doubled
            kernel_size=kernel_size,
            stride=2,  # Upsampling by a factor of 2
        ...
        # Batch normalization (optional but helps stabilize training)
        if bn:
            self.bn = nn.BatchNorm2d(2 * n_feat)
        else:
            self.bn = None
    ...
    def forward(self, x):
        # Reshape tensor for the transposed convolution
        x = x.permute(0, 1, 3, 2)  # Change from (batch, channels, height, width)

        # Apply quaternion deconvolution (upsampling)
        ...
        print(x.shape)
        # Convert back the format
        x = x.permute(0, 1, 3, 2)  # Back to (batch, channels, width, height)
