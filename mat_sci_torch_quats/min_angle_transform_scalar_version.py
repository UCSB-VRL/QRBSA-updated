...
import torch
from mat_sci_torch_quats.quats_old import hadamard_prod, conjugate, outer_prod, fz_reduce, safe_arccos
...
class GradientApproxInvCos():
	...
	def __call__(self, input):
		...
		if self.mode:
			...
			y_rot = torch.zeros_like(input)
			print("self.mode is True")
			# check if the input is < -1 or > 1 and pause the code if it is.
			if torch.any(input <= -1) or torch.any(input >= 1):
				#import pdb; pdb.set_trace()
				pass
			else:
			...
			with torch.no_grad():
				print("self.mode is False")
				# import pdb; pdb.set_trace()
				# clamp the input to be between -1 and 1
				input_temp = torch.clamp(input, -1, 1)
				y_out = 2 * torch.arccos(input_temp)
				
		
		# CHECK IF THIS MAKES TRAIN AND VAL MATCH.
...
class MinimumAngleTransformation(torch.nn.Module):
	...
	def __init__(self, mode):
		...
		self.dist = GradientApproxInvCos(beta= 0.01, mode = self.mode)
	...
	def forward(self, qSR, qHR, syms):
		...
		T_scalar_max, _ =  torch.max(T_syms_scalar, dim=-1)

		if torch.isnan(T_scalar_max).any() or torch.isinf(T_scalar_max).any():
			print("T_scalar_max contains NaNs or Infs BEFORE passing to GradientApproxInvCos!")
			import pdb; pdb.set_trace()
		 
		# check if any scalar is greater than 1 or less than -1
		#if torch.any(T_scalar_max <= -1) or torch.any(T_scalar_max >= 1):
			#import pdb; pdb.set_trace()
