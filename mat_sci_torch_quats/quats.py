...
def kernel_misorientation_deviation(qSR, qHR, angles):
    ...
    q_patches = F.unfold(qHRperm, kernel_size=K).view(B, C, K * K, H - 2, W - 2)
    angles_patches = F.unfold(angles.unsqueeze(1), kernel_size=K).view(B, K * K, H - 2, W - 2)
