...
class checkpoint():
    def __init__(self, args):
        ...
        if args.load == '.':
            if args.save == '.':
                args.save = now
            self.dir = './Quaternion_experiments/' + args.save
        else:
            self.dir = './Quaternion_experiments/' + args.load
    ...
    def quaternion_to_ipf_color(self, quat, ref_dir=np.array([0, 0, 1])):
        """
        Convert a quaternion to an IPF color based on a reference direction similar to Dream3D coloring.
        """
        r = R.from_quat(quat)
        crystal_direction = r.apply(ref_dir)
        crystal_direction /= np.linalg.norm(crystal_direction)
        x, y, z = np.abs(crystal_direction)
        
        # Convert to HSV-like coloring
        hue = np.arctan2(y, x) / (2 * np.pi) + 0.5
        saturation = np.linalg.norm([x, y]) / np.linalg.norm([x, y, z])
        value = z  # Brighter for higher Z component
        ...
        for idx, file_name in enumerate(filenames):
            ...
            if self.args.n_colors != 3:
                for ch_num, channel in enumerate(channels):
                    ...
                    for a, img, title in zip(axes.flatten(), save_list, postfix):
                ...
                img_arr = save_list[0][idx].cpu().numpy()
                img_numpy = (np.transpose(img_arr, (1, 2, 0)) + 1) / 2.0 * 255.0
                img_numpy = np.clip(img_numpy, 0, 255).astype(np.uint8)
                plt.imsave(f'{filename}.png', img_numpy)

            # Generate Dream3D-like IPF Maps
            ipf_map_x = np.zeros((*img_arr.shape[:2], 3), dtype=np.uint8)
            ipf_map_y = np.zeros((*img_arr.shape[:2], 3), dtype=np.uint8)
            ipf_map_z = np.zeros((*img_arr.shape[:2], 3), dtype=np.uint8)

            for i in range(img_arr.shape[0]):
                for j in range(img_arr.shape[1]):
                    quat = img_arr[i, j, :]
                    ipf_map_x[i, j] = self.quaternion_to_ipf_color(quat, ref_dir=np.array([1, 0, 0]))
                    ipf_map_y[i, j] = self.quaternion_to_ipf_color(quat, ref_dir=np.array([0, 1, 0]))
                    ipf_map_z[i, j] = self.quaternion_to_ipf_color(quat, ref_dir=np.array([0, 0, 1]))

            plt.imsave(f'{filename}_ipf_x.png', ipf_map_x)
            plt.imsave(f'{filename}_ipf_y.png', ipf_map_y)
            plt.imsave(f'{filename}_ipf_z.png', ipf_map_z)

    def save_results_old(self, filenames, save_list, postfix, scale, epoch, dataset='val'):
